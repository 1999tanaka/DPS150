...
```text
V(t) = (B + A/2) + (B - A/2) sin(2πt/T)
```

| 項目 | デフォルト |
| --- | ---: |
| A Start / End / Step | 2.0 / 14.0 / 0.1 |
| Formula Base B | 7.0 V |
| Periods | 1 / 5 / 10 s |
| Cycles / Period | 3 |
...
数式中の2つの`7`は、画面の`Formula Base B`から共通して変更できます。Aは整数スケーリングで生成するため、0.1の繰り返し加算による浮動小数点誤差を避けています。各電圧指令は`performance.now()`による実経過時間から計算し、タイマー遅延による周期ずれを累積させません。
